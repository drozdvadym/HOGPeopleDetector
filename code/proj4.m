...
close all
clear
VLFEAT_SETUP_PATH = 'D:\home\devel\src\VLFeatBin\toolbox\vl_setup';
run(VLFEAT_SETUP_PATH)

[~,~,~] = mkdir('visualizations');

data_path = '../data/pedestrian'; %change if you want to work with a network copy
train_path_pos = fullfile(data_path, 'pos'); %Positive training examples. 36x36 head crops
...
feature_params.detect_window_size = [64 128];
feature_params.template_size = 64 * 128;
feature_params.hog_cell_size = 8;
feature_params.hog_num_bins = 9;
feature_params.hog_block_type = 'R-HOG';
feature_params.hog_block_size = 16;
feature_params.hog_block_stride = 8;
feature_params.hog_bin_range = 180;

%% Step 1. Load positive training crops and random negative examples
%YOU CODE 'get_positive_features' and 'get_random_negative_features'

features_pos = get_positive_features( train_path_pos, feature_params, 'png' );
